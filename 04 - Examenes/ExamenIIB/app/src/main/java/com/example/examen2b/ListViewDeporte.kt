...
class ListViewDeporte : AppCompatActivity() {
    var posicionItem = 0
    var idProducto = ""
    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        return when (item.itemId){
            R.id.mn_editar_res -> {
                ...
                irActividad(EditarDeporte::class.java, reseniaProductoElegido)
                return true
            }
    ...
    override fun onCreateContextMenu( menu: ContextMenu?, v: View?, menuInfo: ContextMenu.ContextMenuInfo? ) {
        ...
        FirestoreDeporte.consultarReseniasProductos(idProducto) { reseniaProductoElegido = it[posicionItem].id }
    }
    ...
    private fun cargarResenias(){
        ...
        idProducto = intent.getStringExtra("idProducto")!!
        FirestoreDeporte.consultarReseniasProductos(idProducto!!){
            println(it.size)
            if (it != null) {
    ...
    fun eliminarResenia (){
        FirestoreDeporte.eliminarResenia(reseniaProductoElegido)
        cargarResenias()
    }
    ...
    fun agregarResenia(){
        idProducto = intent.getStringExtra("idProducto")!!
        irActividad(CrearDeporte::class.java, idProducto!!)
        cargarResenias()
    }
